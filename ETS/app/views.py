...
def history(request):
    if request.user.is_authenticated:
        ...
        return render(request, 'User/bookinghistory/history.html', context)
    messages.success(request, "Please login")
    return redirect('/')
    ...
    if request.method == 'POST':
        ...
        if request.user.is_authenticated:
            ...
            if event.objects.filter(cname=cname).exists() and event.objects.filter(cdate=cdate).exists():
                userdata = event.objects.filter(
                    username=username, cname=cname, type=type, cdate=cdate)
                userdata.delete()
                messages.success(request, 'Event deleted Sucessfully')
                return render(request, 'User/eventdelete/delete.html')
        ...
        if request.user.is_authenticated:
            if type == "Caterings":
                ...
                if catering.objects.filter(sname=cname).exists() and catering.objects.filter(namemanu=oname).exists():
                    userdata = catering.objects.filter(
                        username=username, sname=cname, type=type, namemanu=oname)
                    print(userdata)
                    userdata.delete()
                    messages.success(request, 'Event deleted Sucessfully')
            ...
            if type == "Musicical concert":
                ...
                if Musicalconcertm.objects.filter(conname=cname).exists() and Musicalconcertm.objects.filter(name=oname).exists():
                    userdata1 = Musicalconcertm.objects.filter(
                        username=username, conname=cname, type1=type, name=oname)
                    print(userdata1)
                    userdata1.delete()
                    messages.success(request, 'Event deleted Sucessfully')
                    return render(request, 'Dealer/eventdelete/delete.html')
            ...
            if type == "Wedding Hall":
                ...
                if weddinghalls.objects.filter(wname=cname).exists() and weddinghalls.objects.filter(name=oname).exists():
                    userdata2 = weddinghalls.objects.filter(
            ...
            if type == "Birthday Party":
                ...
                if BirthdayParty.objects.filter(birthdayname=cname).exists() and BirthdayParty.objects.filter(birthdayame=oname).exists():
                    userdata3 = BirthdayParty.objects.filter(
...
def faqs(request):
    return render(request, 'User/faqs/faqs.html')
...
def cateringbook(request):
    if request.user.is_authenticated:
        ...
        cont1 = {'kl': allevent1}
        return render(request, 'User/dealers/cateringbook.html', cont1)
    messages.success(request, "PLEASE LOGIN!")
    return redirect('/')
...
def Musicbook(request):
    if request.user.is_authenticated:
        ...
        cont2 = {'pl': allevent2}
        return render(request, 'User/dealers/concertbook.html', cont2)
    messages.success(request, "PLEASE LOGIN!")
    return redirect('/')
...
def Birthdaybook(request):
    if request.user.is_authenticated:
        ...
        cont4 = {'il': allevent4}
        return render(request, 'User/dealers/birthdaybook.html', cont4)
    messages.success(request, "PLEASE LOGIN!")
    return redirect('/')
...
def weddingbook(request):
    if request.user.is_authenticated:
        ...
        cont3 = {'ol': allevent3}
        return render(request, 'User/dealers/weddingbook.html', cont3)
    messages.success(request, "PLEASE LOGIN!")
    return redirect('/')
...
def userregisteruser(request):
    if request.method == 'POST':
        ...
        user3 = User.objects.create_user(
            first_name=fname, last_name=lname, username=username, password=passwd, email=email, date_joined=date)
        user3.save()
        print('user created')
        return redirect('/userlogin')
...
def dealerregisteruser(request):
    if request.method == 'POST':
        ...
        user = User.objects.create_user(first_name=fname, last_name=lname, username=username,
                                        password=passwd, email=email, date_joined=date, is_staff=True)
        user.save()
        print('user created')
        return redirect('/dealerlogin')
...
def event1(request):
    if request.method == 'POST':
        ...
        print(typeofevent, Name, Date, starttime,
              endtime, venue, food, selectdealer)
        book = event(type=typeofevent, cname=Name, cdate=Date, cstime=starttime,
                     cetime=endtime, cvenue=venue, cfood=food, sdealer=selectdealer, username=username)
        book.save()
...
def Caterings(request):
    if request.method == 'POST':
        ...
        cat = catering(type=stype, sname=Servicename, namemanu=Name1,
                       pricemanu=PPrice, mobile=Mobile, emailmanu=Email, username=username)
        cat.save()
        messages.success(request, "Successfully added ")
        return render(request, 'Dealer/events/caterings.html')
    if request.user.is_authenticated:
        return render(request, 'Dealer/events/caterings.html')
    else:
        messages.success(request, "PLEASE LOGIN!")
...
def MusicConcerts(request):
    if request.method == 'POST':
        ...
        if request.user.is_authenticated:
            username = request.user.username
        print(stype1, cname, Name, address, City,
              totprice, Mobile, Email, username)
        Music = Musicalconcertm(type1=stype1, conname=cname, name=Name, Address=address,
                                City=City, Tprice=totprice, mobile=Mobile, email=Email, username=username)
        Music.save()
        messages.success(request, "Successfully added ")
        return render(request, 'Dealer/events/Musicconcerts.html')
    if request.user.is_authenticated:
        return render(request, 'Dealer/events/Musicconcerts.html')
    else:
        messages.success(request, "PLEASE LOGIN!")
...
def weddinghall1(request):
    if request.method == 'POST':
        ...
        if request.user.is_authenticated:
            username = request.user.username
        print(stype2, cname, Name, address, City,
              totprice, Mobile, Email, username)
        weddinghall1 = weddinghalls(type2=stype2, wname=cname, name=Name, Address=address,
                                    City=City, price=totprice, mobile=Mobile, email=Email, username=username)
        weddinghall1.save()
        messages.success(request, "Successfully added ")
        return render(request, 'Dealer/events/weddinghall.html')
    if request.user.is_authenticated:
        return render(request, 'Dealer/events/weddinghall.html')
    else:
        messages.success(request, "PLEASE LOGIN!")
...
def Birthday(request):
    if request.method == 'POST':
        ...
        if request.user.is_authenticated:
            username = request.user.username
        print(stype3, venue, bname, Address,
              City, Price, Phone, email, username)
        party = BirthdayParty(type3=stype3, birthdayname=venue, birthdayame=bname, birthdayAddress=Address,
                              birthdayCity=City, birthdayprice=Price, birthdaymobile=Phone, birthdayemail=email, username=username)
        party.save()
        messages.success(request, "Successfully added ")
        return render(request, 'Dealer/events/Birthday.html')
    if request.user.is_authenticated:
        return render(request, 'Dealer/events/Birthday.html')
    else:
        messages.success(request, "PLEASE LOGIN!")
...
def contactform(request):
    if request.method == 'POST':
        ...
        contactform = contact1(Name=name, email=emailS,
                               State=stateS, Subject=subjectS, username=username)
        contactform.save()
...
def dealercontacts1(request):
    if request.method == 'POST':
        ...
        contactform = dcontact(DName=dname, Demail=demailS,
                               DState=dstateS, DSubject=dsubjectS, username=username)
        contactform.save()
        messages.success(request, "Message sent.")
